...
def record_update_date(subscription_name: str, current_date: str) -> None:
    """
    记录订阅更新日期到临时文件（延迟更新，最后一次性处理）
    
    Args:
        subscription_name: 订阅名称（如 danfeng, v2nodes, nodefree 等）
        current_date: 当前日期（格式：YYYY-MM-DD）
    """
    temp_file = '.github/temp_update_dates.txt'
    ...
    with open(temp_file, 'a', encoding='utf-8') as f:
        f.write(f"{subscription_name}|{current_date}\n")
    
    print(f"[{subscription_name}] 更新日期已记录: {current_date}")


def finalize_readme_dates() -> None:
    """
    根据临时文件中的记录，一次性更新 README.md 中的所有日期
    """
    temp_file = '.github/temp_update_dates.txt'
    
    if not os.path.exists(temp_file):
        print("未找到更新日期临时文件，跳过 README.md 更新")
        return
    ...
    with open(temp_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if line:
                parts = line.split('|')
                if len(parts) == 2:
                    subscription_name, date = parts
                    updates[subscription_name.strip()] = date.strip()
    ...
    for line in lines:
        # 检查该行是否包含任何需要更新的订阅名称
        updated_line = line
        for subscription_name, current_date in updates.items():
            display_name = name_mapping.get(subscription_name.lower(), subscription_name)
